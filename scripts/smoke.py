...
REQUIRED_IMPORTS = [
    ...
    "fastapi",
    "uvicorn",
    # Vector stack (compiler[vector] extras)
    "faiss",
...
def check_cli_tools() -> bool:
    ...
    if not ok:
        print(
            "       hint: re-run `pip install -e \"compiler[validation,vector]\"` "
...
def check_specs() -> bool:
    specs = REPO_ROOT / "compiler" / "src" / "ontology_mapper" / "specs"
    ...
    print(
        ...
        "under compiler/src/ontology_mapper/specs/. If they're missing, "
...
def check_pytest_runs() -> bool:
    """Run the compiler test suite minus the Docker integration tests."""
    cmd = [
        sys.executable, "-m", "pytest", str(REPO_ROOT / "compiler"),
        "--ignore", str(REPO_ROOT / "compiler" / "tests" / "test_kg_integration_neo4j.py"),
        "--ignore", str(REPO_ROOT / "compiler" / "tests" / "test_kg_integration_cypher.py"),
        "--ignore", str(REPO_ROOT / "compiler" / "tests" / "test_kg_integration_sparql.py"),
    ...
    emit("compiler pytest", ok, last_line)
    if not ok:
