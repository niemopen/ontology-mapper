<script>
  ...
  import { mountUserButton, mountOrgSwitcher, unmountOrgSwitcher } from "../lib/auth.js";
  ...
  onMount(() => {
    if (userButtonEl) mountUserButton(userButtonEl);
    if (orgSwitcherEl) mountOrgSwitcher(orgSwitcherEl);
  });

  onDestroy(() => {
    if (orgSwitcherEl) unmountOrgSwitcher(orgSwitcherEl);
  });
</script>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex items-center justify-between h-14">
      <div class="flex items-center gap-4">
        ...
        <div bind:this={orgSwitcherEl}></div>
      </div>
