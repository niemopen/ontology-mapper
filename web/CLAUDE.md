...
- Edit files in `../compiler/` and `../runner/` when needed
...
- **Auth** (Clerk): JWT-based, verified on every API call
- No additional LLM calls — all LLM work happens in the `compiler` and `runner` subdirs
