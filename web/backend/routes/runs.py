"""Pipeline run management routes — all scoped by Clerk organization."""
...
def _read_state(run_dir: Path) -> dict | None:
    state_file = run_dir / ".compiler-state.json"
...
async def list_runs(
    ...
    for d in sorted(runs_dir.iterdir(), reverse=True):
        if d.is_dir() and (d / ".compiler-state.json").exists():
        ...
        result = subprocess.run(
            ...
            env={
                **__import__("os").environ,
                "SEMANTIC_COMPILER_RUNS_DIR": str(org_runs),
            },
        )
...
def _run_pipeline_background(run_id: str, run_dir: Path, org_runs_dir: Path, start_stage: int = 1):
    ...
    env = {
        **__import__("os").environ,
        "SEMANTIC_COMPILER_RUNS_DIR": str(org_runs_dir),
    }
