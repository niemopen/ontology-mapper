...
> **Repo**: OntologyMapperRunner | **Version**: 5.0 | **Updated**: 2026-04-09
...
3. **Pipeline tools**: `ontology-mapper` package installed with all extras (`pip install -e "../compiler[validation,vector]"`)
4. **Runs directory**: `SEMANTIC_COMPILER_RUNS_DIR` set so pipeline tools find run artifacts in this repo
5. **Target ontology**: Must be specified via `--target-ontology` / `--target-version` (e.g., `niem` / `6.0`)

```bash
cd C:/dev/OntologyMapperRunner
...
python runner_tools/run_pipeline.py \
    --run-dir .compiler-runs/redvale_20260409-120000 --from-stage 6
```
...
1. **Environment first.** Set `SEMANTIC_COMPILER_RUNS_DIR` before anything else.
2. **Find the run directory.** `ls -d .compiler-runs/*/ | sort`
