...
| Artifact | Contents |
|----------|----------|
| `governance/version-manifest.json` | Version, target ontology, compiler version, generation history with `mappingStats` (`{action: count}`), pipeline timing (per-stage durations + total) |
...
`run_pipeline.py` marks each stage complete via `om-pipeline mark-complete --stage N` after verification passes. This records `started_at` and `completed_at` in `.compiler-state.json`. Stage 8 reads these timings and includes them in `version-manifest.json`.
