...
export SEMANTIC_COMPILER_RUNS_DIR=C:/dev/OntologyMapperRunner/.compiler-runs
...
run_dir=$(ls -d .compiler-runs/*/ | sort | tail -1 | sed 's:/$::')

# Or target a specific org's most recent run:
run_dir=$(ls -d .compiler-runs/{org}_*/ | sort | tail -1 | sed 's:/$::')
...
- Pipeline state lives in `.compiler-runs/{run_id}/.compiler-state.json` (per-run)
...
- Run artifacts live in `.compiler-runs/{run_id}/`
- Edge package lives at `.compiler-runs/{run_id}/edge-package/`
...
| `om-pipeline rerun` | `--run-dir`, `--organization`, `--stage` | `om-pipeline rerun --run-dir .compiler-runs/x` |
...
| `om-extract` | `--run-dir`, `--package` | `om-extract --run-dir .compiler-runs/x --package sources/org/domain/` |
...
| `om-build-strategy` | `--run-dir`, `--catalog` | `om-build-strategy --run-dir .compiler-runs/x` |
| `om-batch-search` | `--run-dir`, `--top-k`, `--min-score-ratio` | `om-batch-search --run-dir .compiler-runs/x` |
| `om-entropy` | `--run-dir` | `om-entropy --run-dir .compiler-runs/x` |
| `om-orchestrate-eval` | `--run-dir`, `--concurrency`, `--model`, `--max-retries`, `--dry-run` | `om-orchestrate-eval --run-dir .compiler-runs/x` |
| `om-collect-alignments` | `--run-dir`, `--allow-pending` | `om-collect-alignments --run-dir .compiler-runs/x` |
| `om-build-matrix` | `--run-dir` | `om-build-matrix --run-dir .compiler-runs/x` |
| `om-generation-audit` | `--run-dir` | `om-generation-audit --run-dir .compiler-runs/x` |
| `om-generate-ontology` | `--run-dir`, `--package-dir` | `om-generate-ontology --run-dir .compiler-runs/x` |
| `om-package-artifacts` | `--run-dir`, `--package-dir` | `om-package-artifacts --run-dir .compiler-runs/x` |
| `om-generate-kg` | `--run-dir`, `--package-dir` | `om-generate-kg --run-dir .compiler-runs/x` |
| `om-validate` | `--run-dir`, `--package-dir` | `om-validate --run-dir .compiler-runs/x` |
| `om-finalize` | `--run-dir`, `--package-dir` | `om-finalize --run-dir .compiler-runs/x` |
| `verify_stage_outputs.py` | `--run-dir`, `--stage` | `python runner_tools/verify_stage_outputs.py --run-dir .compiler-runs/x --stage 3` |
| `_present_and_apply_human_review.py` | `--run-dir` | `python runner_tools/_present_and_apply_human_review.py --run-dir .compiler-runs/x` |
| `feedback_report.py` | `--run-dir` | `python runner_tools/feedback_report.py --run-dir .compiler-runs/x` |

| `run_feedback.py` | `log`/`show`, `--run-dir`, `--stage`, `--type`, etc. | `python runner_tools/run_feedback.py log --run-dir .compiler-runs/x --stage 3 --type bug --component "..." --description "..." --target pipeline` |
...
| `om-residual-entropy` | `--run-dir` | `om-residual-entropy --run-dir .compiler-runs/x` |
