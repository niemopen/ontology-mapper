...
**Pipeline tools are installed as**: the `ontology-mapper` package (`pip install -e ../compiler`)
...
| [AGENTS/Pipeline__AZURE_DEPLOYMENT.md](AGENTS/Pipeline__AZURE_DEPLOYMENT.md) | Cloud deployment shape: stage-to-compute mapping, Service Bus fan-out, state externalization, prerequisites, cost shape |
| [AGENTS/Pipeline__CLOUDFLARE_FRONTEND.md](AGENTS/Pipeline__CLOUDFLARE_FRONTEND.md) | Cloudflare-hosted frontend variant: Svelte UI + Clerk auth + Stripe billing, calling the Azure backend over authenticated HTTPS |
