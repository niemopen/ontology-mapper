...
Tests use monkeypatched asyncio.create_subprocess_exec to avoid invoking
the real claude CLI. The mock simulates the subprocess response envelope
that claude -p --output-format json returns.
...
def _make_mock(stdout=b"", stderr=b"", returncode=0, hang=False):
    """Create a monkeypatch-ready mock for create_subprocess_exec."""
...
class TestProvenanceMetadata:
    ...
    def test_type_provenance_fields_present(self, monkeypatch):
        ...
        assert result["evaluatedBy"] == "sonnet"
    ...
    def test_property_provenance_fields_present(self, monkeypatch):
        ...
        assert "evaluatedAt" in result
        assert result["evaluatedBy"] == "opus"
    ...
    def test_model_name_recorded(self, monkeypatch):
        ...
        assert result["evaluatedBy"] == "haiku"
...
class TestErrorHandling:
    ...
    def test_nonzero_exit_code(self, monkeypatch):
        ...
        with pytest.raises(EvaluationError, match="exited 1"):
    ...
    def test_malformed_json(self, monkeypatch):
        ...
        with pytest.raises(EvaluationError, match="Malformed JSON"):
            asyncio.run(evaluate_file(_type_doc(), _context()))
