...
via ``claude -p``, validates responses, and writes evaluations back.

Usage:
    om-orchestrate-eval --run-dir {run_dir} [--concurrency 8] [--model sonnet]
"""
...
async def process_file(
    ...
    async with semaphore:
        ...
        while attempts <= max_retries:
            try:
                evaluation = await evaluate_file(doc, context, model=model)
...
async def run_evaluations(
    ...
    tasks = [
        process_file(path, context, model, max_retries, semaphore, counter)
        for path in files
    ]
    ...
    _save_orchestration_config(run_dir, model, concurrency, max_retries)
    print(f"  Done.")
...
def _save_orchestration_config(
    run_dir: Path, model: str, concurrency: int, max_retries: int,
) -> None:
    """Write orchestration config to .compiler-state.json."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run bounded per-file semantic evaluation via claude -p"
    )
