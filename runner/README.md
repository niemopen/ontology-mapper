# OntologyMapperRunner

Automated pipeline runner for the [OntologyMapper](https://github.com/CardamomCosmos/SemanticCompiler). `run_pipeline.py` executes all pipeline stages end-to-end: running `om-*` CLI tools, using `claude -p` for bounded LLM evaluation (Stage 3) and natural-language review interpretation (Stage 5), and producing aligned edge packages for target ontologies (NIEM, SALI/FOLIO, etc.).
...
.compiler-runs/                    # Pipeline run artifacts (gitignored)
```
...
pip install -e "../compiler[validation,vector]"

# Set runs directory
export SEMANTIC_COMPILER_RUNS_DIR=$(pwd)/.compiler-runs
...
Catalog generation is owned by the `ontology-mapper` package. See the [compiler README — Reference Catalogs](../compiler/README.md#reference-catalogs) for the authoritative details (API sources, enrichment flags, offline mode, regeneration policy).
...
Developer mode (see `CLAUDE.md`): build and maintain tools in `runner_tools/` and `../compiler/`. Pipeline runs are executed via `run_pipeline.py`, not by the LLM.
