...
Each pipeline run gets an isolated directory under `.compiler-runs/`:

```
.compiler-runs/
  {org}_{YYYYMMDD-HHMMSS}/
    .compiler-state.json     # Pipeline state (inputs, stages, progress)
...
The `SEMANTIC_COMPILER_RUNS_DIR` env var overrides the `.compiler-runs/` root.
...
`.compiler-state.json` and validates that `target_ontology` and
...
key in `.compiler-state.json`. All stage tools call `load_config()`.
...
- `om-pipeline` creates the run directory and writes `.compiler-state.json`
...
- **State file is the source of truth.** Stage tools read inputs from
  `.compiler-state.json`, not from CLI args (except `--run-dir`).
...
- **Hardcoding run paths**: Use `resolve_run_dir(cli_arg=...)`, never
  construct paths to `.compiler-runs/` manually.
- **Reading global state**: There is no global state file. Every run has
  its own `.compiler-state.json`.
