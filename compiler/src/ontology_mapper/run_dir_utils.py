...
.compiler-state.json from the working directory.  State lives inside
each run directory: .compiler-runs/{run_id}/.compiler-state.json
...
STATE_FILENAME = ".compiler-state.json"
RUNS_ROOT = Path(os.environ.get("SEMANTIC_COMPILER_RUNS_DIR", ".compiler-runs"))


def resolve_specs_dir():
    """Return the specs directory path.

    Checks SEMANTIC_COMPILER_SPECS_DIR env var first, falls back to
    the specs/ directory inside this package.
    """
    env = os.environ.get("SEMANTIC_COMPILER_SPECS_DIR")
    if env:
        return Path(env)
...
def resolve_run_dir(cli_arg=None, org=None):
    ...
    if not cli_arg:
        raise FileNotFoundError(
            ...
            "(e.g., .compiler-runs/redvale_20260406-045218)."
        )
