...
Below is a clean definition of the **semantic compiler** in terms of **inputs, outputs, decision rules, and artifact types**.

## 1. Tool definition

The tool is a **NIEM-aware semantic compiler** powered by Claude Code as the real-time semantic reasoning engine.
...
These are the materials the compiler analyzes to understand the domain.
...
These tell the compiler what slice of the domain to model.
...
These tell the compiler what the output must support operationally.
...
These tell the compiler how aggressively to reuse NIEM and when to extend.
...
These tell the compiler what to emit.
...
These explain what the compiler found.
...
These are the core rules that make the compiler more than a lookup utility.
...
The compiler must decide how entities are identified and merged.
...
Multi-tenancy is deferred — the compiler currently processes one domain for one organization at a time.
...
**Mandatory review gate**: After Stage 4 (Decide), the compiler must present the complete concept inventory and mapping matrix to the user for review before proceeding to Stage 5 (Generate). No artifact generation occurs without user approval. See [AGENTS/OntologyMapper__PROJECT_SPECIFIC.md](AGENTS/OntologyMapper__PROJECT_SPECIFIC.md) Section I for the full review protocol.
...
The compiler should emit artifacts in several formal categories.
...
Present the complete concept inventory and mapping matrix to the user for approval. The compiler does not proceed until the user has reviewed and approved the results.
